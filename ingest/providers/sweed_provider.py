# ingest/providers/sweed_provider.py
"""
Sweed menu provider - fetches all products from Sweed-powered dispensary sites.

Requires store_id which can be:
1. Passed explicitly via provider_metadata in the dispensary record
2. Discovered via discover_sweed.py and stored in provider_metadata

This implementation:
- Fetches category filters
- Iterates ALL categories
- Paginates fully per category
- De-dupes products across categories
- Returns items in the exact schema expected by run_scrape.py
"""

from __future__ import annotations

import json
from typing import Any, Dict, List, Optional

from ingest.providers.sweed_api import (
    get_categories,
    fetch_all_products_for_category,
    normalize_sweed_product,
)

# ---------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------

def _extract_store_id_from_metadata(
    provider_metadata: Optional[Dict[str, Any]]
) -> Optional[str]:
    """Extract store_id from provider_metadata JSON."""
    if not provider_metadata:
        return None

    if isinstance(provider_metadata, str):
        try:
            provider_metadata = json.loads(provider_metadata)
        except (json.JSONDecodeError, TypeError):
            return None

    return provider_metadata.get("store_id") or provider_metadata.get("storeId")


def _infer_sale_type(menu_url: str) -> str:
    """Infer sale type from URL patterns."""
    url_lower = (menu_url or "").lower()
    if "medical" in url_lower or "med" in url_lower:
        return "Medical"
    return "Recreational"


def _to_raw_item(normalized: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert normalized Sweed product to the format expected by run_scrape.py.

    Required keys:
      name, category, brand, price, discount_price,
      discount_text, provider_product_id, raw
    """
    return {
        "name": normalized.get("name"),
        "category": normalized.get("category"),
        "brand": normalized.get("brand"),
        "price": normalized.get("price"),
        "discount_price": normalized.get("discount_price"),
        "discount_text": normalized.get("discount_text"),
        "provider_product_id": normalized.get("provider_product_id"),
        "raw": normalized.get("raw", {}),
    }


# ---------------------------------------------------------------------
# Main entry point (called by run_scrape.py)
# ---------------------------------------------------------------------

def fetch_menu_items(
    *,
    menu_url: str,
    store_id: Optional[str] = None,
    provider_metadata: Optional[Dict[str, Any]] = None,
    sale_type: Optional[str] = None,
) -> List[Dict[str, Any]]:
    """
    Fetch all menu items from a Sweed-powered dispensary.

    Args:
        menu_url: Dispensary menu URL
        store_id: Sweed store ID (optional if present in provider_metadata)
        provider_metadata: Dict containing store_id and other config
        sale_type: "Recreational" or "Medical" (auto-detected if not provided)

    Returns:
        List of menu items ready for RawMenuItem insertion
    """

    # -----------------------------------------------------------------
    # Resolve required context
    # -----------------------------------------------------------------

    resolved_store_id = store_id or _extract_store_id_from_metadata(provider_metadata)
    if not resolved_store_id:
        raise ValueError(
            "store_id is required for Sweed provider. "
            "Run discover_sweed.py and store the result in dispensary.provider_metadata."
        )

    resolved_sale_type = sale_type or _infer_sale_type(menu_url)

    # -----------------------------------------------------------------
    # Step 1: Fetch category filters
    # -----------------------------------------------------------------

    categories = get_categories(
        store_id=resolved_store_id,
        sale_type=resolved_sale_type,
    )

    # -----------------------------------------------------------------
    # Step 2: Fetch products (category-aware, paginated, de-duped)
    # -----------------------------------------------------------------

    all_items: List[Dict[str, Any]] = []
    seen_product_ids: set[str] = set()

    # If no categories are returned, fall back to a single unfiltered pull
    if not categories:
        products = fetch_all_products_for_category(
            store_id=resolved_store_id,
            category_or_filters={},   # Empty filters = full catalog
            sale_type=resolved_sale_type,
            page_size=60,
        )

        for p in products:
            pid = str(p.get("id") or p.get("productId") or "")
            if pid and pid in seen_product_ids:
                continue
            if pid:
                seen_product_ids.add(pid)

            normalized = normalize_sweed_product(p)
            all_items.append(_to_raw_item(normalized))

        return all_items

    # Normal case: iterate categories
    for cat in categories:
        cat_name = None
        cat_filter = None

        if isinstance(cat, dict):
            cat_name = cat.get("name") or cat.get("categoryName")

            # Preferred: explicit filter object
            if isinstance(cat.get("filter"), dict):
                cat_filter = cat["filter"]
            elif isinstance(cat.get("filters"), dict):
                cat_filter = cat["filters"]
            else:
                # Fallback: build filter from ID
                cat_id = (
                    cat.get("id")
                    or cat.get("categoryId")
                    or cat.get("category_id")
                )
                if cat_id:
                    cat_filter = {"category": [int(cat_id)]}
        else:
            # cat may be a bare category ID
            cat_filter = {"category": [int(cat)]}

        if not cat_filter:
            continue

        products = fetch_all_products_for_category(
            store_id=resolved_store_id,
            category_or_filters=cat_filter,
            sale_type=resolved_sale_type,
            page_size=60,
        )

        for p in products:
            pid = str(p.get("id") or p.get("productId") or "")
            if pid and pid in seen_product_ids:
                continue
            if pid:
                seen_product_ids.add(pid)

            normalized = normalize_sweed_product(
                p,
                category_name=cat_name,
            )
            all_items.append(_to_raw_item(normalized))

    return all_items
